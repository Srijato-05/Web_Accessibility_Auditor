  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 pb-32 relative min-h-screen">
      ...
      <header className="mb-8 border-b border-surface-border pb-8">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors mb-6 text-sm font-bold focus:ring-2 focus:ring-primary outline-none">
           <ArrowLeft size={16} /> Back
        </button>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div>
               <div className="flex items-center gap-3 mb-2">
                  <div className="bg-primary/10 text-primary p-2 rounded-md border border-primary/25">
                     <Network size={24} />
                  </div>
      ...
      <div className="flat-panel p-8 border-primary/20 bg-surface-highlight max-w-5xl mx-auto">
         <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex gap-2">
               {data.recommended && <span className="text-xs bg-secondary/15 text-secondary border border-secondary/25 px-3 py-1 rounded-full uppercase tracking-widest font-bold">Recommended</span>}
            </div>
         </div>
         ...
         <div className="p-6 bg-[#111111] rounded-md border border-surface-border mb-6 text-sm text-on-surface-variant font-mono relative overflow-hidden">
            <p className="mb-3 text-[#E0E0E0]"><span className="text-secondary font-bold mr-2">{'>'} Vertex</span> <span className="bg-secondary/15 text-secondary px-2 py-0.5 rounded border border-secondary/25">{data.component_id}</span></p>
      ...
      {isResolved && (
        <div className="max-w-5xl mx-auto mt-8 flex flex-col gap-6">
          <div className="flat-panel p-6 border-secondary/35 bg-surface">
