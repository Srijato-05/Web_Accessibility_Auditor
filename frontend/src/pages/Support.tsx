...
import { LifeBuoy, ArrowLeft, Send, CheckCircle, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-on-surface p-8 max-w-4xl mx-auto pb-32">
       <header className="mb-8 border-b border-surface-border pb-8">
        ...
        <div className="flex items-center gap-3">
            ...
            <h1 className="text-display font-medium text-2xl font-heading">Support Center</h1>
        </div>
        <p className="text-on-surface-variant text-sm mt-2 tracking-wide">Open a priority technical request vector with system operations.</p>
      </header>
         ...
         <form onSubmit={handleSubmit} className="glass-panel p-8 bg-surface-container-low shadow-ambient">
            ...
            <div className="space-y-6">
               <div>
                  ...
                  <input type="text" placeholder="John Doe" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-surface py-3 px-4 rounded-md border border-surface-border text-sm focus:outline-none focus:border-primary/50 text-on-surface transition-colors shadow-inner" />
               </div>
               
               <div>
                  <label className="text-xs uppercase tracking-widest font-bold text-on-surface-variant block mb-2">Issue Vector</label>
                  <select value={formData.issue} onChange={e => setFormData({...formData, issue: e.target.value})} className="w-full bg-surface py-3 px-4 rounded-md border border-surface-border text-sm focus:outline-none focus:border-primary/50 text-on-surface transition-colors shadow-inner appearance-none cursor-pointer">
                     ...
                     <option value="Misc Heuristics">Miscellaneous Heuristics</option>
                  </select>
               </div>
               
               <div>
                  <label className="text-xs uppercase tracking-widest font-bold text-on-surface-variant block mb-2">Payload Description</label>
                  <textarea placeholder="Describe the structural failure..." value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} className="w-full bg-surface py-3 px-4 rounded-md border border-surface-border text-sm focus:outline-none focus:border-primary/50 text-on-surface transition-colors min-h-32 shadow-inner"></textarea>
               </div>
            </div>

            <button type="submit" disabled={isSubmitting} className="w-full primary-btn mt-8 flex items-center justify-center gap-2 py-4 cursor-pointer disabled:opacity-50 transition-all">
